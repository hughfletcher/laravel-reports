    ...
    <tr class="treegrid-{{ str_random(6) }}{{ isset($parent) ? ' treegrid-parent-' . $parent : ''}}">
        @if(count($child->header_errors) > 0)
            <td colspan="4">
                ...
                <a href="{{ route('reports.show', ['format' => 'html', 'report' => $child->report_path]) }}">
                    {{ $child->name or $child->report_path }}
                </a> has {{ count($child->header_errors) }} errors.
            </td>
        @else
            <td><span class="glyphicon glyphicon-file" aria-hidden="true"></span><a href="{{ route('reports.show', ['format' => 'html', 'report' => $child->report_path]) }}">{{ $child->name }}</a></td>
            <td>{{ $child->description }}</td>
